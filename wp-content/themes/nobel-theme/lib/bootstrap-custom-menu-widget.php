...
load_theme_textdomain( 'nobeltheme', get_template_directory() . '/languages' );
...
class bootstrap_custom_menu extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			__('Boot Strap Menu', 'nobeltheme'),
			array('description' => __('A custom menu widget that uses the wp_bootstrap_navwalker', 'nobeltheme'))
    ...
    function form( $instance ) {
     ...
     $defaults = array ( 'title' => __('Menu', 'nobeltheme'), 'menuname' => __('Menu Name', 'nobeltheme'), 'menutype' => 'nav-tabs');
     $instance = wp_parse_args( (array) $instance, $defaults );
    ?>
    <p>
        <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title','nobeltheme'); ?>:</label>
        <input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" style="width:100%;" />
    </p>

     <p>
            <label for="<?php echo $this->get_field_id( 'menuname' ); ?>"><?php _e('Menu','nobeltheme'); ?>:</label>
            <select id="<?php echo $this->get_field_id( 'menuname' ); ?>" name="<?php echo $this->get_field_name( 'menuname' ); ?>" style="width:100%;" type="text">
                <?php
                    ...
                    $menus = get_terms( 'nav_menu', array( 'hide_empty' => false ) );
                        ...
                        echo "<option value=''>". __('Default All Pages','nobeltheme') ."</option>";
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'menutype' ); ?>"><?php _e('Menu Type','nobeltheme'); ?>:</label>
